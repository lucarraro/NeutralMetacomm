...
if (save_alphaDiv_time){alphaDiv_time <- matrix(0,nTimesteps/100,nNodes)} # initialize alphaDiv_time
...
for (iter in 1:nTimesteps){
  ...
  if ((iter %% 100)==0){ # write progress (and possibly evaluate alphaDiv_time)
    cat(sprintf("%.1f%% done  - Elapsed time: %.2f s\r",iter/nTimesteps*100,difftime(Sys.time(),t0)))
...
ll <- list(speciesDB=speciesDB, speciesCount=speciesCount) # store species database and total species richness
